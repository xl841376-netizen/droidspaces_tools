/*
 * TypeScript reference for droidspaces_tools v0.2.0
 * 
 * This file documents the types used but IS valid JavaScript.
 * It mirrors packages/droidspaces.js (the authoritative source).
 * Type information is in JSDoc comments for IDE support.
 * 
 * Build: scripts/compile.py copies packages/droidspaces.js to output.
 */


/**
 * @typedef {{ success: boolean, output: string, exit_code: number, timed_out?: boolean }} ShellResult
 * @typedef {{ name: string, container: string, sid: string, hasTmux: boolean, lastOutput: string, createdAt: number }} SessionState
 */
...
METADATA
{
    ...
    "tools": [
        ...
        { "name": "read_screen", "description": { "zh": "【终端读取】读取交互式终端的可见屏幕内容。⚡只读不写。❌需要执行命令用 exec_code。", "en": "[Terminal Read] Read visible screen content from terminal. ⚡Read only. ❌ Use exec_code to run commands." }, "parameters": [ { "name": "session_id", "type": "string", "required": true, "description": "会话 ID" } ] },        { "name": "start", "description": { "zh": "【启动容器】启动一个新 Droidspaces 容器。⚡只启动不执行命令。启动后如需执行命令走 exec_code。", "en": "[Start Container] Start a new Droidspaces container. ⚡Starts only. Use exec_code to run commands." }, "parameters": [ { "name": "name", "type": "string", "required": true, "description": "容器名称" }, { "name": "rootfs", "type": "string", "required": false, "description": "rootfs 目录路径" }, { "name": "rootfs_img", "type": "string", "required": false, "description": "rootfs ext4 镜像路径" }, { "name": "hostname", "type": "string", "required": false, "description": "容器主机名" }, { "name": "conf", "type": "string", "required": false, "description": "配置文件路径" }, { "name": "net", "type": "string", "required": false, "description": "网络模式 host/nat/none" }, { "name": "volatile", "type": "boolean", "required": false, "description": "临时模式" }, { "name": "bind_mounts", "type": "string", "required": false, "description": "绑定挂载 src:dst" }, { "name": "port", "type": "string", "required": false, "description": "端口转发 HOST:CONT[/proto]" }, { "name": "extra_args", "type": "string", "required": false, "description": "额外参数如 --gpu" } ] },
